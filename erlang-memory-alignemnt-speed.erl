    ...
    receive
        ...
        len ->
            ListOfTime = lists:map(fun(_) ->
                                        Items = items(3*1000),
                                        T1 = erlang:now(),
                                        %L = queue:join(Queue, queue:from_list(Items)),
                                        %L = queue:join(queue:from_list(Items), Queue),
                                        %L = queue:from_list(lists:append(queue:to_list(Queue), Items)),
                                        T2 = erlang:now(),
        ...
        hibernate ->
            ...
            erlang:hibernate(testn, loop, [Queue]);
        {From, ping} ->
            From ! pong,
            loop(Queue);
        _ -> ok
