...
static int process_stoped(struct trace_process *process, int signal) {
	int pid = process->pid;
	int inject_signal = 0;
	...
	case SIGTRAP | 0x80: { // assuming PTRACE_O_SYSGOOD
		...
		if (syscall_entry != !process->within_syscall) {
			ERROR("syscall entry - exit desynchronizaion");
		} else {
	...
	case SIGTRAP | (PTRACE_EVENT_CLONE << 8): {
		...
		struct trace_process *child_process =
			trace_process_new(process->trace, child_pid);
		process->trace->callback(child_process, TRACE_ENTER,
					 (void*)child_pid, process->trace->userdata);
		break; }
...
static int trace_waitpid(struct trace *trace, int pid_hint) {
	int inject_signal = 0;
	int status = 0;
	int pid = 0;

	int r = waitpid(pid_hint, &status, WNOHANG | __WALL);
	if (r == -1) {
		if (errno != ECHILD)
			PFATAL("waitpid()");
		return 0;
	...
	if (!process) {
		ERROR("waitpid() returned unknown process pid=%i status=0x%x",
		      pid, status);
		return 1;
	}
	...
	if (!process->initialized) {
		...
		if (WIFSTOPPED(status)) {
			...
			inject_signal = process_stoped(process, signal);
		} else
		if (WIFSIGNALED(status) || WIFEXITED(status)) {
			struct trace_exitarg exitarg;
			if (WIFSIGNALED(status)) {
				exitarg = (struct trace_exitarg){TRACE_EXIT_SIGNAL,
								 WTERMSIG(status)};
			} else {
				exitarg = (struct trace_exitarg){TRACE_EXIT_NORMAL,
								 WEXITSTATUS(status)};
			}
			...
			ERROR("pid=%i status 0x%x not understood!", pid, status);
		}
	}
...
void trace_read(struct trace *trace) {
	...
	if (sinfo_sz != 1) {
		/* TODO: */
		ERROR("got more than one object on signal_fd %i!", sinfo_sz);
	}

	/* First, try to poke pid's that are likely waitpid-able */
	int i;
	for (i = 0; i < sinfo_sz; i++) {
		trace_waitpid(trace, sinfo[i].ssi_pid);
	}

	/* Next, as signals may be coalesced - do waitpid for any
	 * child. */
	while (1) {
		r = trace_waitpid(trace, -1);
		if (!r) break;
	}
}
